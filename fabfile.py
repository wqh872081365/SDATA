...
@roles('apps')
def start_rqworker():
    sudo("supervisorctl start sdata_rq_default sdata_rq_high sdata_rq_low sdata_rq_add_spider "
...
def restart_rqworker():
    sudo("supervisorctl restart sdata_rq_default sdata_rq_high sdata_rq_low sdata_rq_add_spider "
         "sdata_rq_spider_pipeline sdata_rq_scheduler sdata_rq_spider_status "
         "sdata_rq_worker_number_control sdata_rq_proxy_valid")
...
@roles('db')
def start_pgadmin4():
    run(env_root + "/pgadmin4/bin/python3.6 " + env_root +
...
@roles('apps')
def update_settings():
    sudo("scp -f " + code_path + "/deploy/local_settings_aliyun.py " + "%s@%s:" % ((env.user, env.host)) + code_path +
         "/SDATA/local_settings.py")
